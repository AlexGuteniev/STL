...
void constructors() {
    ...
    // 4. From consant and obj
    {
        ...
        function_ref fn10(constant_arg<&global_object_nx::fn>, glob_nx);
        assert(fn10(0x22, 0xFF) == 0x88);

        function_ref fn11(constant_arg<&global_object_nx::m>, glob_nx);
        assert(fn11() == 0x55);

        int v = 3;
        function_ref fn12(constant_arg<::fn_nx>, v);
        assert(fn12(4) == 7);
#endif // __cpp_noexcept_function_type
    }
...
#ifdef __cpp_noexcept_function_type
        function_ref<unsigned(unsigned, unsigned)> fn3(constant_arg<&global_object_nx::fn>, &glob_nx);
        assert(fn3(0x99, 0x88) == 0x44);
#endif // __cpp_noexcept_function_type
    }
}
...
void copy_and_assign() {
    ...
    // When implementing LWG-4264, 5 will be expected
    assert(ref_copy_dif_sign(2, 3) == -1);
    ...
    // When implementing LWG-4264, 6 will be expected
    assert(ref_nx_copy(2, 3) == 5);
#endif // __cpp_noexcept_function_type
}
