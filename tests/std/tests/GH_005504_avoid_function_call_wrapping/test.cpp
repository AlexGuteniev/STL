...
int main() {
    ...
    // function_ref in other wrappres. See LWG-4264 -- we are not implementing it yet.
    // When/if we can implement this, the thunk will not be unified, but we will save invoke.
    alloc_checker{0}, test_wrapped_ref_call<function<fn_type>, function_ref<fn_type>>(1);
    ...
    // function_ref in function_ref. See LWG-4264 -- we are not implementing it yet.
    ...
    alloc_checker{0}, test_wrapped_ref_call<function_ref<fn_type>, function_ref<fn_type_c>>(1);
#ifdef __cpp_noexcept_function_type
    alloc_checker{0}, test_wrapped_ref_call<function_ref<fn_type>, function_ref<fn_type_nx>>(1);
#endif // defined(__cpp_noexcept_function_type)

    // Other wrappers in function_ref.These cannot unwrap due to the reference semantics, see LWG-4264 discussion.
